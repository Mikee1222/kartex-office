...
import {
  isLegacyColorId,
  toLegacyColorId,
} from "@/lib/products/master-variant/legacy-color-options";
import type {
...
function toCreatedVariantRow(
  ...
  return {
    ...
    color: colorName,
    colorId,
    sku: inserted.sku?.trim() ?? skuFallback,
...
export async function createMasterVariant(
  supabase: SupabaseClient,
  input: CreateMasterVariantInput,
): Promise<{ variant: CreatedMasterVariantRow } | { error: string }> {
  const {
    master,
    widthCm,
    heightCm,
  ...
  if (isLegacyColorId(colorId)) {
    return {
      variant: toCreatedVariantRow(
        inserted,
        colorName,
        toLegacyColorId(colorName),
        subcategory,
        roundedStock,
        internalPriceEur,
        skuResult.sku,
  ...
  return {
    variant: toCreatedVariantRow(
      { ...inserted, stock: totalStock ?? roundedStock },
      colorName,
      colorId,
      subcategory,
      roundedStock,
