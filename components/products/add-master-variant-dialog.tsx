...
import { resolveDefaultSubcategory } from "@/lib/products/master-variant/adapters";
import {
  createMasterVariant,
...
} from "@/lib/products/master-variant/variant-catalog-options";
...
export type AddMasterVariantDialogMode = "website" | "products";
...
export function AddMasterVariantDialog({
  ...
  React.useEffect(() => {
    ...
    async function loadOptions() {
      ...
      setDefaultSubcategory(
        resolveDefaultSubcategory(
          master,
          subcategories.options.map((option) => option.value),
        ),
      );
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    let stock = 0;
    if (isProductsMode) {
      const trimmedStock = startingStock.trim();
      const parsedStock = Number.parseInt(trimmedStock, 10);
      if (
        trimmedStock === "" ||
        !Number.isFinite(parsedStock) ||
        parsedStock < 0
      ) {
        setError("Εισάγετε έγκυρο αρχικό απόθεμα (≥ 0).");
        return;
      }
      stock = parsedStock;
    }

    let internalPriceEur: number | null = null;
    if (!isProductsMode) {
    ...
    >
      <form onSubmit={(event) => void handleSubmit(event)}>
        <DialogBody className="space-y-4">
          ...
          {isProductsMode ? (
            <div className="space-y-2">
              <Label htmlFor="variant-starting-stock">Αρχικό απόθεμα</Label>
              <Input
                id="variant-starting-stock"
                type="number"
                min={0}
                value={startingStock}
                onChange={(event) => setStartingStock(event.target.value)}
                disabled={pending}
              />
            </div>
          ) : (
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-600">
            ...
            {defaultSubcategory ? (
              <p className={isProductsMode ? undefined : "mt-1"}>
